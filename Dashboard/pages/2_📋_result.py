...
if not result_true():
    ...
    result = load_result()
    data = load_data(str(result['file_name'][1]))
    ...
    with tab1:
        with st.container():
            ...
            #                       'idx' : 'Time'})
            # 정상과 비정상 데이터 분리
            result['정상'] = result['final_score']
            result['비정상'] = result['final_score']

            # 정상 데이터에서 비정상 구간을 NaN으로 설정
            result.loc[result['pred'] == 1, '정상'] = None
            # 비정상 데이터에서 정상 구간을 NaN으로 설정
            result.loc[result['pred'] == 0, '비정상'] = None

            # Plotly 그래프 생성
            fig = px.line(result, x='idx', y=['정상', '비정상'],
                        labels={'value': 'Anomaly-score', 'variable': '탐지결과', 'idx': 'Time'},
                        color_discrete_map={'정상': 'blue', '비정상': 'red'})
