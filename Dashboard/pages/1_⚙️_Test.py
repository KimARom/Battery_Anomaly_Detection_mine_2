...
from libraries.load_data import save_uploaded_file
import subprocess
import warnings
...
with col1:
    ...
    if start_1:
        ...
        save_uploaded_file('./Dashboard', csv_file)
    
    
    cmd = [f"{sys.executable}", os.path.join(os.getcwd(), 'Modeling', 'run.py'), 'test.csv']
    test = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, stdin=subprocess.DEVNULL)

# 같은 자리에 버튼이 바뀜 근데 그냥 버튼 새로 만드는게 좋을 듯.
...
if start and st.session_state.test:
    data = load_data(str(csv_file.name))
    with st.sidebar:
        before.empty()
